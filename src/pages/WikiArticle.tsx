import { ArrowLeftIcon, BookOpenIcon, ClockIcon, EyeIcon, PencilIcon, TagIcon, TrashIcon, UserIcon } from '@heroicons/react/24/solid';
  ...
  if (notFound || !page) {
    return (
      <div className="min-h-full p-6 flex flex-col items-center justify-center text-center gap-4">
        ...
        >
          <ArrowLeftIcon className="w-4 h-4" /> กลับไป Wiki
        </Link>
      </div>
    );
  }
    ...
    <div className="min-h-full p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          >
            <ArrowLeftIcon className="w-3.5 h-3.5" /> Back to Wiki
          </Link>
          {isAdmin && (
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            >
              <TagIcon className="w-3 h-3" /> {page.category}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground pb-5 border-b border-border">
            {page.author_name && (
              <span className="flex items-center gap-1.5">
                <UserIcon className="w-3.5 h-3.5" /> {page.author_name}
              </span>
            )}
            <span className="flex items-center gap-1.5">
            ...
            <span className="flex items-center gap-1.5">
              <EyeIcon className="w-3.5 h-3.5" /> {page.view_count} views
            </span>
          </div>
        </div>
