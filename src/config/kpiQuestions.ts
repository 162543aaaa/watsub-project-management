...
export const ROLEKEY_TO_DB_ROLE: Record<RoleKey, string | null> = {
  // Keep non-outsource roles on stable in-code templates for now.
  // Their DB templates can be enabled later after complete seeding.
  ta: null,
  hafeez: null,
  sumayna: null,
  outsource: "outsource",
...
const ROLE_ALIASES: Record<RoleKey, string[]> = {
  ta: ["tarmisi", "ต้า", "director"],
  hafeez: ["hafeez", "ฮาฟีซ", "production"],
  sumayna: ["sumayna", "สุไมยนา", "strategy"],
...
const OS_SELF: KPISection[] = [
  sec("job_performance", "Job Performance", C.job, "45%", [
    ...
    ...rates("os_self_job", "job_performance", [
      "งานตรงตาม brief ที่ได้รับโดยไม่ต้องอธิบายซ้ำ",
      "ความรับผิดชอบต่อไฟล์งานและ asset ที่ได้รับ",
      "แจ้งล่วงหน้าเมื่อรู้ว่าจะส่งงานไม่ทัน",
    ]),
  ]),
  sec("competency", "Competency", C.comp, "40%", [
    ...rates("os_self_comp", "competency", [
      ...
      "เข้าใจ visual direction หรือ tone ของ WatSUB!",
    ]),
  ...
  sec("collaboration", "Collaboration", C.team, "15%", [
    ...
    ...texts("os_self_collab_text", [
      ...
      "อยากทำงานกับ WatSUB! ในโปรเจกต์ถัดไปมั้ย เพราะอะไร",
    ]),
  ]),
];
...
const OS_PEER: KPISection[] = [
  sec("collaboration", "Collaboration", C.team, "peer", [
    ...rates("os_peer_collab", "collaboration", [
      ...
      "ทำงานด้วยแล้ว friction น้อย ไม่กินเวลาทีม",
      "อยากร่วมงานด้วยในโปรเจกต์ถัดไปมั้ย",
    ]),
...
const OS_SUP: KPISection[] = [
  ...
  sec("competency", "Competency", C.comp, "40%", rates("os_sup_comp", "competency", [
    ...
    "เข้าใจ direction และ standard ของ WatSUB!",
...
export async function loadKpiFormConfig(
  ...
  (data as DbTemplateRow[]).forEach((row, i) => {
    ...
    if (!["auto", "rate", "text", "hidden"].includes(normalizedType)) return;
    ...
    const t = normalizedType as QuestionType;
  ...
  const dbRenderableCount = dbConfig.sections.reduce((sum, s) =>
    sum + s.questions.filter((q) => q.type === "auto" || q.type === "rate" || q.type === "text").length, 0);
